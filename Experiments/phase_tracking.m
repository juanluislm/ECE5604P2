...
for i=(N_corr):k:(fix(length(b_rxs_not_merged)/k)*k)-1
    ...
    b_rxs_not_merged((i+1):(i+k)) = b_rxs_not_merged((i+1):(i+k)).*exp(1j*my_delta./(1:k));%((i+1):(i+k)) = b_rxs_not_merged((i+1):(i+k)).*exp(1j*my_delta./(k:-1:1));
