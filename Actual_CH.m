...
syms_up3 = [syms_up3 zeros(1,(2*NSPS1)+1000)];
...
tx_fc3 = tx3.*(exp(2*pi*1i*12000/44100.*nn3));
...
tx2r = real(tx_fc2);
tx3r = [real(tx_fc3) zeros(1,28)];
% tx4r = real(tx_fc4);
tx5r = real(tx_fc5);
tx6r = real(tx_fc6);

txs=tx1r+tx2r+tx3r+tx5r+tx6r; % sum of txs
...
hold on
plot(fftshift(f2r)/1000,fftshift(db(fft2r)),'y'); 
...
plot(fftshift(fts)/1000,fftshift(db(ffts)),'r'); %PSD of sum tx
