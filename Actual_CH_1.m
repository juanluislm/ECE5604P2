...
Nsym = 10000 ; % number of symbols; larger number may take time but give better resolution
T = 1; %symbol time
...
txs=txs.*(exp(-2*pi*1i*4500/44100.*nn6));
%txs=real(txs);
...
b_txs = filter(b,1,[txs zeros(1,200)]);
b_rxs = filter(shape,1,b_txs);
...
slice_ids1 = (m*k+(b_ord/2)+1):NSPS:length(b_rxs) ;%(2*m*k+1):NSPS:length(frx1)
slice_ids1 = slice_ids1(1:Nsym) ; % limit to comparison with number of symbols sent
b_rxs1 = b_rxs(slice_ids1) ;

figure(3)
plot(real(b_rxs1),imag(b_rxs1),'o') ;
title('decision variable, k= 28, m=8, Nsym=10000') ;
xlabel('I') ;
ylabel('Q') ;
grid on
...
BER = ((sum(abs(sign(real(b_rxs1(err_ids)))-real(syms1(err_ids))) ...
   + abs(sign(imag(b_rxs1(err_ids)))-imag(syms1(err_ids)))))/2)/(2*Nsym);
